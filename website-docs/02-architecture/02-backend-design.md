...
| `Auth(...)` | auth.go | 核心认证，顺序：① 免认证白名单 / OPTIONS；② JWT（`Authorization: Bearer`）；若 JWT 有效但请求上的 CAS uid cookie 与用户 `cas_user_id` 不一致则**忽略该 JWT**（换账号后本地 token 未刷新），继续后续通道；无 CAS cookie 不算冲突，纯 Bearer 客户端可用。③ API Key（`X-API-Key`）。④ 可选 `nxin_cas_auth`（`auth_cas.go`）：用 `_cas_sid`/`_cas_uid` cookie 校验并绑定用户。支持 `X-Tenant-ID` 切换租户（`IsTenantAccessible` 三层校验：自有租户/跨租户超管/active membership），`resolveTenantRole` 解析租户内角色。写入 context：`TenantIDContextKey`、`TenantInfoContextKey`、`UserContextKey`、`UserIDContextKey`、`TenantRoleContextKey`、`SystemAdminContextKey`、`PrincipalContextKey` 等 |
