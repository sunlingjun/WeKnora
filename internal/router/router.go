...
func NewRouter(params RouterParams) *gin.Engine {
	...
	r.Use(cors.New(cors.Config{
		...
		AllowHeaders:     []string{"Origin", "Content-Type", "Accept", "Authorization", "X-API-Key", "X-Open-Retrieve-Api-Key", "X-Request-ID", "X-Tenant-ID", "X-Embed-Session"},
	...
	RegisterEmbedPublicRoutes(r, params.EmbedChannelHandler, params.EmbedChannelService, params.TenantService, singleRedisClient(params.RedisClient), params.FileService)

	// 认证中间件
	r.Use(middleware.Auth(params.TenantService, params.UserService, params.TenantMemberService, params.CASAuthService, params.RedisClient, params.Config))
...
func RegisterKnowledgeBaseRoutes(r *gin.RouterGroup, handler *handler.KnowledgeBaseHandler, g *rbacGuards) {
	...
	kb := r.Group("/knowledge-bases")
	{
		...
		// 置顶/取消置顶知识库 — Viewer+ 且对 KB 有 read 权限
