...
func (s *userService) GetCurrentUser(ctx context.Context) (*types.User, error) {
	// Try UserInfoContextKey first (set by Auth middleware)
	user, ok := ctx.Value(types.UserInfoContextKey).(*types.User)
	if !ok {
		// Fallback to UserContextKey for backward compatibility
