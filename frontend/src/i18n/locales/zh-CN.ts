export default {
  ...
  tenantInvitation: {
    ...
    enterpriseLink: {
      cellTitle: '企业邀请链接',
      cellEmpty: '尚无企业成员加入',
      description: '生成一条企业账号加入链接，发到群里即可；对方通过企业 CAS 登录后加入当前空间。链接 {days} 天后过期，或随时可在列表中撤销。',
      generate: '生成链接',
  ...
  tenantMember: {
    ...
    casImport: {
      button: '导入农信用户',
      title: '导入农信用户',
      description: '仅支持已在农信注册的手机号；未注册的行会失败，不影响其他行。',
      roleLabel: '导入后角色',
      fileLabel: '上传表格',
      fileHint: '支持 .xlsx / .csv，识别「手机号」「姓名」列。多余列会忽略。',
      chooseFile: '选择文件',
      phonesLabel: '或粘贴手机号',
      phonesPlaceholder: '每行一个手机号，也可写成「手机号,姓名」',
      ...
      previewTitle: '预览结果',
      resultTitle: '导入结果',
      summaryPreview: '共 {total} 行：将创建账号 {create}，仅加入 {add}，已是成员 {member}，无法导入 {fail}',
      summaryResult: '导入成功 {imported}，跳过 {skipped}，失败 {failed}',
      noRows: '请先上传文件或粘贴手机号。',
      emptyPreview: '没有可展示的行。',
      downloadFailed: '下载失败行',
      unconfigured: '农信用户导入未配置，请联系管理员。',
      tooMany: '单次最多导入 200 行。',
      columns: {
        phone: '手机号',
        name: '姓名',
        casName: '农信姓名',
  ...
  integrations: {
    ...
    api: {
      ...
      copy: '复制',
      copySuccess: '已复制',
      mcpRetrieveUrl: 'MCP 检索地址',
      mcpRetrieveUrlDesc: '远程 Streamable HTTP MCP（知识召回 / wiki 图谱）。需使用具备「检索」能力的 API Key；在 Key 行点击复制 MCP 配置。',
  ...
  knowledgeList: {
    ...
    members: {
      title: '成员管理',
      description: '管理知识库"{name}"的成员',
      searchPlaceholder: '按邮箱或姓名搜索',
      empty: '暂无成员',
      unknownUser: '未知用户',
      joinedAt: '加入时间',
      confirmRemoveTitle: '移除成员',
      confirmRemoveMessage: '确定要移除成员"{name}"吗？',
      actions: {
        setEditor: '设为编辑者',
        setViewer: '设为查看者',
        remove: '移除成员'
  ...
  settings: {
    ...
    eventWebhooks: {
      ...
      noDeliveries: '暂无投递记录',
      exampleLabel: '验签示例',
      exampleDesc: 'HMAC 签的是 timestamp + "." + 原始 body（必须用原始字节，不要先 parse JSON）。源文件用 5 分钟下载票拉取；验签通过后立刻返回 200。',
      exampleTab: {
        java: 'Java',
        python: 'Python',
        go: 'Go',
      ...
      payloadLabel: '事件载荷',
      payloadDesc: '接收方读到的 POST body。按 type 分支；同一 resource 下 data 的 key 集合固定。',
      payloadHintCreated: '文件型 created 带 5 分钟下载票。URL / 手工知识 available=false，用 data.source。此时不可检索。',
      payloadHintParsed: 'parse_status=completed。enable_status=enabled 表示流水线打开检索。即使还没收到 created 也可以 upsert。',
      payloadHintParseFailed: 'parse_status=failed，看 error_message。源文件通常仍可 download 排障。',
      payloadHintDeleted: 'deleted=true，不要再 download。按 knowledge_id 删本地副本。',
      payloadHintBatchDeleted: '不为每条 id 再发 knowledge.deleted。knowledge_ids 最多 100 条，超出则同一次删除再发下一条。truncated 恒为 false。',
      payloadHintKbCreated: '建库节点。visibility 为 private 或 shared。',
      payloadHintKbDeleted: '整库对本空间不可用。不要再等 N 条 knowledge.deleted，也不要 download。',
      payloadHintMemberAdded: '本工作空间成员加入，不是共享空间/组织名册。',
      payloadHintMemberRemoved: 'reason 为 removed（踢出）或 left（自己离开）。该用户失去本空间知识访问；库和文档还在。',
      payloadHintTest: '设置页发送测试。data 只有 resource + ok，不要用知识字段去解。',
      copy: '复制',
      copied: '已复制',
      copyFailed: '复制失败',
    },
