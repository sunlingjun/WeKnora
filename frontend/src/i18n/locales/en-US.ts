export default {
  ...
  settings: {
    ...
    eventWebhooks: {
      ...
      sendTest: 'Send test',
      testQueued: 'Test queued, delivering…',
      ...
      exampleDesc: 'HMAC is timestamp + "." + raw body (use the raw bytes; do not re-serialize JSON). Download files with the 5-minute ticket; return 200 immediately after verifying.',
      exampleTab: {
        java: 'Java',
        python: 'Python',
        go: 'Go',
      ...
      payloadDesc: 'POST body the receiver sees. Branch on type; data keys are fixed per resource.',
      payloadHintCreated: 'File created includes a 5-minute download ticket. URL / manual knowledge has available=false; use data.source. Not searchable yet.',
      payloadHintParsed: 'parse_status=completed. enable_status=enabled means the pipeline opened retrieval. You may upsert even if created has not arrived.',
      payloadHintParseFailed: 'parse_status=failed; see error_message. The source file can usually still be downloaded for debugging.',
      payloadHintDeleted: 'deleted=true; do not download. Remove the local copy by knowledge_id.',
      payloadHintBatchDeleted: 'No per-id knowledge.deleted. knowledge_ids is at most 100; extra ids arrive in further events for the same delete. truncated is always false.',
      payloadHintKbCreated: 'Create the knowledge-base node. visibility is private or shared.',
      payloadHintKbDeleted: 'The whole knowledge base is gone for this workspace. Do not wait for N knowledge.deleted events, and do not download.',
      payloadHintMemberAdded: 'A user joined this workspace, not a shared-space / org roster.',
      payloadHintMemberRemoved: 'reason is removed (kicked) or left. The user loses access to this workspace; bases and documents remain.',
      payloadHintTest: 'Sent from Send test. data is only resource + ok; do not parse knowledge fields.',
      copy: 'Copy',
      copied: 'Copied',
      copyFailed: 'Copy failed',
    },
  ...
  createChat: {
    title: 'Hi, I am ZSK — your knowledge, within reach',
