export default {
  ...
  tenantInvitation: {
    ...
    enterpriseLink: {
      ...
      description: 'Generate a multi-use enterprise join link you can drop in a group chat. Invitees sign in with their enterprise CAS account and join this workspace. Expires in {days} days, or revoke it any time from the list.',
      generate: 'Generate link',
      resultTitle: 'Enterprise invite link ready',
      resultBody: 'Copy the link below and share it. Invitees will sign in with their enterprise account and join this workspace. The link is also saved in the list — you can re-copy or revoke it later.',
      revokeConfirm: "Revoking will block anyone who hasn't joined yet from using this link. Generate a new one to re-share."
    },
    errors: {
  ...
  tenantMember: {
    ...
    casImport: {
      button: 'Import NXIN users',
      title: 'Import NXIN users',
      description: 'Only phone numbers already registered in NXIN can be imported. Failed rows do not block the rest.',
      roleLabel: 'Role after import',
      fileLabel: 'Upload spreadsheet',
      fileHint: 'Accepts .xlsx / .csv. Columns 「手机号」/phone and 「姓名」/name are recognized; extra columns are ignored.',
      chooseFile: 'Choose file',
      phonesLabel: 'Or paste phone numbers',
      phonesPlaceholder: 'One phone number per line, or "phone,name"',
      ...
      summaryPreview: '{total} rows: create {create}, add only {add}, already members {member}, cannot import {fail}',
      summaryResult: 'Imported {imported}, skipped {skipped}, failed {failed}',
      noRows: 'Upload a file or paste phone numbers first.',
      emptyPreview: 'No rows to show.',
      downloadFailed: 'Download failed rows',
      unconfigured: 'NXIN user import is not configured. Contact an administrator.',
      tooMany: 'At most 200 rows per import.',
      columns: {
        phone: 'Phone',
        name: 'Name',
        casName: 'NXIN name',
      ...
      accountYes: 'Exists',
      accountNo: 'None',
      inTenantYes: 'Already a member',
      inTenantNo: 'Not a member',
      action: {
        create_user: 'Create and add',
        add_member: 'Add only'
      },
      status: {
        importable: 'Importable',
        already_member: 'Already a member',
        not_found: 'Not found in NXIN',
        name_mismatch: 'Name mismatch',
        invalid_phone: 'Invalid phone',
        ambiguous: 'Ambiguous match',
        local_conflict: 'Account conflict',
        failed: 'Failed',
        skipped: 'Skipped',
        imported: 'Imported'
      }
  ...
  integrations: {
    ...
    api: {
      ...
      mcpRetrieveUrl: 'MCP retrieve URL',
      mcpRetrieveUrlDesc: 'Remote Streamable HTTP MCP (knowledge retrieve / wiki graph). Use an API Key with retrieve capability; copy MCP config from the Key row.',
  ...
  createChat: {
    title: 'Привет, я ZSK — ваши знания всегда под рукой',
  ...
  settings: {
    ...
    eventWebhooks: {
      ...
      testQueued: 'Тест поставлен в очередь, идёт доставка…',
      testDelivered: 'Доставлено',
      testFailed: 'Тест не удался',
      statusPending: 'Доставляется',
      statusFailed: 'Ошибка',
      noDeliveries: 'Доставок пока нет',
      exampleLabel: 'Проверка подписи',
      exampleDesc: 'HMAC — timestamp + "." + raw body (сырые байты, без повторной сериализации JSON). Файлы скачиваются 5-минутным билетом; после проверки сразу верните 200.',
      exampleTab: {
        java: 'Java',
        python: 'Python',
        go: 'Go',
      ...
      payloadLabel: 'Тело события',
      payloadDesc: 'POST body, которое видит получатель. Ветвитесь по type; набор ключей data фиксирован для каждого resource.',
      payloadHintCreated: 'Для файлового created есть 5-минутный билет. Для URL/ручной записи available=false — берите data.source. Пока нельзя искать.',
      payloadHintParsed: 'parse_status=completed. enable_status=enabled значит, что конвейер открыл поиск. Можно upsert даже без created.',
      payloadHintParseFailed: 'parse_status=failed, смотрите error_message. Исходный файл обычно ещё можно скачать для отладки.',
      payloadHintDeleted: 'deleted=true; больше не скачивайте. Удалите локальную копию по knowledge_id.',
      payloadHintBatchDeleted: 'Отдельных knowledge.deleted на каждый id нет. knowledge_ids — не больше 100; остаток придёт следующими событиями того же удаления. truncated всегда false.',
      payloadHintKbCreated: 'Создайте узел базы знаний. visibility — private или shared.',
      payloadHintKbDeleted: 'Вся база недоступна этому пространству. Не ждите N событий knowledge.deleted и не скачивайте файлы.',
      payloadHintMemberAdded: 'Пользователь этого пространства, не общего/организационного списка.',
      payloadHintMemberRemoved: 'reason: removed (исключили) или left (ушёл сам). Пользователь теряет доступ; базы и документы остаются.',
      payloadHintTest: 'Отправляется кнопкой «тест». В data только resource + ok; не разбирайте поля знания.',
      copy: 'Копировать',
      copied: 'Скопировано',
