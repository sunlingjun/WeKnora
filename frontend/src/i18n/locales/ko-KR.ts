export default {
  ...
  createChat: {
    title: '안녕하세요, ZSK입니다 — 당신의 지식을 손끝에',
  ...
  settings: {
    ...
    eventWebhooks: {
      ...
      exampleLabel: '서명 검증',
      exampleDesc: 'HMAC은 timestamp + "." + raw body입니다(원본 바이트를 쓰고 JSON을 다시 직렬화하지 마세요). 파일은 5분 티켓으로 받고, 검증 후 바로 200을 반환하세요.',
      exampleTab: {
        java: 'Java',
        python: 'Python',
        go: 'Go',
      ...
      payloadLabel: '이벤트 페이로드',
      payloadDesc: '수신측이 읽는 POST body입니다. type으로 분기하고, resource마다 data 키 집합은 고정입니다.',
      payloadHintCreated: '파일형 created에는 5분 다운로드 티켓이 있습니다. URL/수동 지식은 available=false이며 data.source를 쓰세요. 아직 검색할 수 없습니다.',
      payloadHintParsed: 'parse_status=completed. enable_status=enabled는 파이프라인이 검색을 연 것입니다. created가 없어도 upsert할 수 있습니다.',
      payloadHintParseFailed: 'parse_status=failed, error_message를 보세요. 원본 파일은 보통 여전히 다운로드할 수 있습니다.',
      payloadHintDeleted: 'deleted=true이며 다시 다운로드하지 마세요. knowledge_id로 로컬 사본을 삭제하세요.',
      payloadHintBatchDeleted: 'id마다 knowledge.deleted를 보내지 않습니다. knowledge_ids는 최대 100개이며 초과분은 같은 삭제의 다음 이벤트로 옵니다. truncated는 항상 false입니다.',
      payloadHintKbCreated: '지식베이스 노드를 만듭니다. visibility는 private 또는 shared입니다.',
      payloadHintKbDeleted: '이 워크스페이스에서 지식베이스 전체가 사라집니다. N개의 knowledge.deleted를 기다리지 말고 다운로드도 하지 마세요.',
      payloadHintMemberAdded: '이 워크스페이스 사용자 추가이며, 공유 공간/조직 명단이 아닙니다.',
      payloadHintMemberRemoved: 'reason은 removed(강제 제거) 또는 left(자진 탈퇴)입니다. 해당 사용자는 이 공간 지식에 접근하지 못하지만 베이스와 문서는 남습니다.',
      payloadHintTest: '보내기 테스트에서 발송됩니다. data는 resource + ok뿐이며 지식 필드로 파싱하지 마세요.',
      copy: '복사',
      copied: '복사됨',
      copyFailed: '복사 실패',
    },
