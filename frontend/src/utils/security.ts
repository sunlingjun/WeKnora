...
export function sanitizeHTML(html: string): string {
  ...
  try {
    ...
    return DOMPurify.sanitize(preparedHTML, DOMPurifyConfig as any) as unknown as string;
  } catch (error) {
...
export function sanitizeMarkdownHTML(html: string): string {
  ...
  try {
    const preparedHTML = protectProviderImageSrcInHTML(html);
    return DOMPurify.sanitize(preparedHTML, markdownDomPurifyConfig as any) as unknown as string;
  } catch (error) {
